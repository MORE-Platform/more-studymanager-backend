...
public class ObservationRepository {
    ...
    private static final String INSERT_NEW_OBSERVATION = "INSERT INTO observations(study_id,observation_id,title,purpose,participant_info,type,study_group_id,properties,schedule,hidden,no_schedule,reminder,milestone_id) VALUES (:study_id,(SELECT COALESCE(MAX(observation_id),0)+1 FROM observations WHERE study_id = :study_id),:title,:purpose,:participant_info,:type,:study_group_id,:properties::jsonb,:schedule::jsonb,:hidden,:no_schedule,:reminder,:milestone_id)";
    private static final String IMPORT_OBSERVATION = "INSERT INTO observations(study_id,observation_id,title,purpose,participant_info,type,study_group_id,properties,schedule,hidden,no_schedule,reminder,milestone_id) VALUES (:study_id,:observation_id,:title,:purpose,:participant_info,:type,:study_group_id,:properties::jsonb,:schedule::jsonb,:hidden,:no_schedule,:reminder,:milestone_id)";
    ...
    private static final String LIST_OBSERVATIONS_FOR_GROUP = """
            ...
            GROUP BY o.study_id, o.observation_id""";
    private static final String UPDATE_OBSERVATION = "UPDATE observations SET title=:title, purpose=:purpose, participant_info=:participant_info, study_group_id=:study_group_id, properties=:properties::jsonb, schedule=:schedule::jsonb, modified=now(), hidden=:hidden, no_schedule=:no_schedule, reminder=:reminder, milestone_id=:milestone_id WHERE study_id=:study_id AND observation_id=:observation_id";
    ...
    private static MapSqlParameterSource toParams(Observation observation) throws JsonProcessingException {
        return toParams(observation.getStudyId())
                ...
                .addValue("milestone_id", observation.getMilestoneId());
    }
